...
data_glass = read_csv(filepath_or_buffer='../Lab_Boosting/Glass.csv', sep=',', header=0)

x_train, x_test, y_train, y_test = split_data(data_glass)

n_trees = range(1, 202, 10)
test_error = []
for n in n_trees:
    model = BaggingClassifier(n_estimators=n).fit(x_train, y_train)
